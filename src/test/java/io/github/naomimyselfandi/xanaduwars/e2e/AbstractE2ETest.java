...
public class AbstractE2ETest extends AbstractIntegrationTest {
    ...
    void setup(TestInfo testInfo, SeededRng random) {
        ...
        testInfo.getTestMethod()
                .map(method -> method.getAnnotation(WithTestUser.class))
    ...
    protected Account createAccount(AccountKind kind, Role... roles) {
        ...
        var authenticationSecret = switch (kind) {
            case HUMAN -> random.nextPlaintextPassword();
            case BOT -> random.nextPlaintextAPIKey();
        };
        var account = accountService.create(username, emailAddress, authenticationSecret).orElseThrow();
    ...
    protected void login(@Nullable Account account) {
        ...
        authorization = account == null ? null : switch (account.authenticationSecret()) {
            case Password _ -> {
                ...
                var token = jwtFactory.generateToken(account, JWTPurpose.ACCESS_TOKEN, JWTClaim.NONE, duration);
                yield "Bearer " + token;
            }
            case APIKey _ -> {
                var id = account.id();
