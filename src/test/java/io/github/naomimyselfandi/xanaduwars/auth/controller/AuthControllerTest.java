...
class AuthControllerTest {
    ...
    void login(RememberMe rememberMe, SeededRng random) {
        ...
        var account = new HumanAccount().id(random.nextUUID());
        when(accountService.find(username, password)).thenReturn(Optional.of(account));
        var accessTokenDuration = Duration.ofMinutes(random.nextInt(5, 60));
        var accessToken = random.nextUUID().toString();
        when(jwtDurationService.duration(account, ACCESS_TOKEN, rememberMe)).thenReturn(accessTokenDuration);
        when(jwtFactory.generateToken(account, ACCESS_TOKEN, NONE, accessTokenDuration)).thenReturn(accessToken);
        var refreshTokenDuration = Duration.ofHours(random.nextInt(24, 576));
        var refreshToken = random.nextUUID().toString();
        when(jwtDurationService.duration(account, REFRESH_TOKEN, rememberMe)).thenReturn(refreshTokenDuration);
        when(jwtFactory.generateToken(account, REFRESH_TOKEN, NONE, refreshTokenDuration)).thenReturn(refreshToken);
    ...
    void login_WhenTheCredentialsAreInvalid_ThenReturnsA401(RememberMe rememberMe, SeededRng random) {
        var username = random.nextUsername();
        var password = random.nextPlaintextPassword();
        when(accountService.find(username, password)).thenReturn(Optional.empty());
        var request = new LoginRequest(username, password);
    ...
    void refresh(RememberMe rememberMe, SeededRng random) {
        ...
        var account = new HumanAccount().id(random.nextUUID());
        when(jwt.getSubject()).thenReturn(account.id().toString());
        when(accountService.find(account.id())).thenReturn(Optional.of(account));
        var accessTokenDuration = Duration.ofMinutes(random.nextInt(5, 60));
        var accessToken = random.nextUUID().toString();
        when(jwtDurationService.duration(account, ACCESS_TOKEN, rememberMe)).thenReturn(accessTokenDuration);
        when(jwtFactory.generateToken(account, ACCESS_TOKEN, NONE, accessTokenDuration)).thenReturn(accessToken);
