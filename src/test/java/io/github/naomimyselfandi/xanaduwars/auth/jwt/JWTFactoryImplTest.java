...
class JWTFactoryImplTest {
    ...
    void setup(SeededRng random) {
        account = new HumanAccount().id(random.nextUUID());
        ...
        when(jwtKeyService.getForSigning(purpose, expiry)).thenReturn(new SecretKeyWithId(secretKey, account.id()));
    ...
    void generateToken(RepetitionInfo repetitionInfo) {
        var expected = """
                ...
                .findFirst()
                .orElseThrow();
        assertThat(fixture.generateToken(account, purpose, claim, duration)).isEqualTo(expected);
    }
