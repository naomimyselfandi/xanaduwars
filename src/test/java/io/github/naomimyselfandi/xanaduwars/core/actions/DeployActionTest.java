...
class DeployActionTest {
    ...
    void setup(SeededRandom random) {
        ...
        fixture = new DeployAction(new Name("A" + random.nextInt(Integer.MAX_VALUE)), TagSet.EMPTY);
    }
    ...
    @EnumSource
    @ParameterizedTest
    void cost(Resource resource, SeededRandom random) {
        var baseCosts = Arrays
                .stream(Resource.values())
                .collect(Collectors.toMap(Function.identity(), _ -> random.nextInt(Integer.MAX_VALUE)));
        var baseCost = baseCosts.get(resource);
        var modifiedCost = random.nextInt(Integer.MAX_VALUE);
        var calculation = new UnitCostCalculation(tile, unitType, resource);
        when(unitType.costs()).thenReturn(baseCosts);
        when(gameState.evaluate(calculation, baseCost)).thenReturn(modifiedCost);
        assertThat(fixture.cost(resource, tile, unitType)).isEqualTo(modifiedCost);
    }
