...
class CommandSequenceForUnitTest {
    ...
    void submit(boolean expected, SeededRng random) throws CommandException {
        var sequence = random.<CommandSequenceForUnit>get();
        when(gameState.submitUnitCommand(sequence.x(), sequence.y(), sequence.unitCommands())).thenReturn(expected);
