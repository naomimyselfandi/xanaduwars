...
import io.github.naomimyselfandi.xanaduwars.core.queries.ActionGroupValidation;
import io.github.naomimyselfandi.xanaduwars.core.queries.ActionValidation;
...
class ActionExecutorImplTest {
    ...
    void execute() {
        ...
        var inOrder = inOrder(player, foo, bar, unit);
        ...
        inOrder.verify(unit).canAct(false);
    }
    ...
    void execute_WhenTheActionGroupIsInvalid_ThenThrows() {
        ...
        verify(foo, never()).execute(unit, fooTarget);
        verify(bar, never()).execute(unit, barTarget);
        verify(unit, never()).canAct(false);
