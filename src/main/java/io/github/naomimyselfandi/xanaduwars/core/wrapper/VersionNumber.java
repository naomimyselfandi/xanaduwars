...
public record VersionNumber(
        @Pattern(regexp = VersionNumber.REGEX) @NotNull @JsonValue String text
) implements StringWrapper, Comparable<VersionNumber> {
    ...
    public int compareTo(VersionNumber that) {
        var thisMatcher = PATTERN.matcher(this.text);
        var thatMatcher = PATTERN.matcher(that.text);
