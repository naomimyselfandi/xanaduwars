...
enum MoveLogicImpl implements MoveLogic {
    ...
    public Execution execute(Path path, Unit unit) {
        ...
        for (var tile : steps) {
            ...
            var baseCost = baseCost(tile.movementTable(), tags);
            speedUsed += gameState.evaluate(new EntryQuery(unit, tile), baseCost);
    ...
    private static double baseCost(Map<Tag, Double> movementTable, TagSet tags) {
        return tags
                .stream()
                .map(movementTable::get)
