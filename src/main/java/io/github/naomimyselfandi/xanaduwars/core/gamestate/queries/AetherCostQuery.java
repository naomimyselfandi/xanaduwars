...
/// A query that calculates an action's aether cost.
public record AetherCostQuery(Element subject, Action action, @Unmodifiable List<Object> targets)
        implements QueryWithTargets<Integer> {

    /// A query that calculates an action's aether cost.
    /// @implSpec This constructor takes an immutable copy of its list argument.
    public AetherCostQuery(Element subject, Action action, List<Object> targets) {
    ...
    public List<Script> prologue() {
        return List.of(action.getAetherCost());
    }
