...
public record DropAction(
...
) implements ActionWithEnumTarget<Unit, Direction> {
    ...
    public boolean test(Unit unit, Direction direction) {
        ...
        return !unit.gameState().evaluate(new EntryQuery(cargo, destination), 0.0).isNaN();
    }
    ...
    public Execution execute(Unit unit, Direction direction) {
        ...
        if (unit.gameState().evaluate(new EntryQuery(cargo, destination), 0.0).isNaN()) {
