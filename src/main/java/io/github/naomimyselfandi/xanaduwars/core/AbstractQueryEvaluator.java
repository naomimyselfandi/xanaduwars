...
public abstract class AbstractQueryEvaluator implements QueryEvaluator {
    ...
    @Override
    public <Q extends Query<V>, V> V evaluate(Q query, V value) {
        return evaluate(query, value, NOOP);
    }

    @Override
    public <V> V evaluate(DefaultQuery<V> query) {
        return evaluate(query, query.defaultValue());
    }

    @Override
    public boolean evaluate(Validation query, Consumer<Rule<?, ?>> callback) {
        return evaluate(query, true, callback);
    }

    private <Q extends Query<V>, V> V evaluate(Q query, V value, Consumer<Rule<?, ?>> callback) {
