...
/// A query that calculates an action's supply cost.
public record SupplyCostQuery(Element subject, Action action, @Unmodifiable List<Object> targets)
        implements QueryWithTargets<Integer> {

    /// A query that calculates an action's supply cost.
    /// @implSpec This constructor takes an immutable copy of its list argument.
    public SupplyCostQuery(Element subject, Action action, List<Object> targets) {
    ...
    public List<Script> prologue() {
        return List.of(action.getSupplyCost());
    }
