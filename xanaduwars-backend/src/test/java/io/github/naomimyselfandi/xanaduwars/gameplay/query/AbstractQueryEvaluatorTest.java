...
class AbstractQueryEvaluatorTest {
    ...
    private record FooQuery(UUID foo, Integer defaultValue) implements Query<Integer> {
    ...
    private record BarQuery(Integer defaultValue) implements Query<Integer> {}
    ...
    private final AbstractQueryEvaluator fixture = new AbstractQueryEvaluator() {
        ...
        protected Stream<Rule<?, ?>> contextualRules(Query<?> query) {
            if (query instanceof FooQuery(var f, var _) && f.equals(foo)) {
    ...
    void evaluate() {
        ...
        assertThat(fixture.evaluate(query)).isEqualTo(4);
    ...
    void evaluate_RespectsShortCircuiting() {
        ...
        when(globalFooRule1.handles(query, 1)).thenReturn(true);
        when(globalFooRule1.handle(query, 1)).thenReturn(-2);
        assertThat(fixture.evaluate(query)).isEqualTo(-2);
