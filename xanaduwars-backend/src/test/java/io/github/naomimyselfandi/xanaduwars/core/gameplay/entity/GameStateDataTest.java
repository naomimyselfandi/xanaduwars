...
class GameStateDataTest {
    ...
    void setup(SeededRng random) {
        t0x0 = random.nextTileData().id(new TileId(0, 0));
        t1x0 = random.nextTileData().id(new TileId(1, 0));
        t2x0 = random.nextTileData().id(new TileId(2, 0));
        t3x0 = random.nextTileData().id(new TileId(3, 0));
        t0x1 = random.nextTileData().id(new TileId(0, 1));
        t1x1 = random.nextTileData().id(new TileId(1, 1));
        t2x1 = random.nextTileData().id(new TileId(2, 1));
        t3x1 = random.nextTileData().id(new TileId(3, 1));
        fixture = new GameStateData()
                .version(random.nextVersion())
                .turn(random.get())
                .playerData(List.of(random.nextPlayerData()))
                .tileData(List.of(t0x0, t1x0, t2x0, t3x0, t0x1, t1x1, t2x1, t3x1))
                .unitData(List.of(random.nextUnitData(), random.nextUnitData()));
    }
    ...
    @CsvSource(textBlock = """
            ...
            0,2
            """)
    void tileDataAt_WhenTheIdIsOutOfBounds_ThenReturnsAnEmptyResult(int x, int y) {
        assertThat(fixture.tileDataAt(new TileId(x, y))).isEmpty();
    }

    @Test
    void unitDataOf() {
        for (var unitData : fixture.unitData()) {
            assertThat(fixture.unitDataOf(unitData.id())).contains(unitData);
        }
    }
    ...
    void unitDataAt() {
        for (var unitData : fixture.unitData()) {
            assertThat(fixture.unitDataAt(unitData.location())).contains(unitData);
        }
    }
    ...
    void testEquals(SeededRng random) {
        ...
        assertThat(fixture.id(random.get())).isSameAs(fixture);
        assertThat(fixture).isNotEqualTo(other);
        assertThat(other.id(random.get())).isSameAs(other);
        assertThat(fixture).isNotEqualTo(other);
        assertThat(other.id(fixture.id())).isSameAs(other);
        assertThat(fixture).isEqualTo(other);
        assertThat(fixture).isNotEqualTo(fixture.id());
    }
